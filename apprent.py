...
st.caption("📈 Investment Return Rate is the expected annual return if you invest the savings instead of buying (e.g., in stocks or mutual funds).")
...
fig, ax = plt.subplots()
ax.plot(year_list, [v / 1_000_000 for v in house_value_list], label="Owner's House Value", marker='o')
ax.plot(year_list, [v / 1_000_000 for v in principal_left_list], label="Owner's Principal Left", marker='x')
ax.plot(year_list, [v / 1_000_000 for v in value_of_house_owned_list], label="Owner's Value of House Owned", marker='x')
ax.plot(year_list, [v / 1_000_000 for v in investment_list], label="Renter's Portfolio Value", marker='x')
ax.set_xlabel("Year")
